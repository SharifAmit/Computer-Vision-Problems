...
int main()
{
    ...
    for(int j=0; j<n;j++){
        if(j>0){
             downsampled = downsampling2d(normalized,2);
        }
        for(int i=0; i<s;i++){
            if(i==0 and j>0){
                sigma = k*sigma;
                cout << sigma << endl;
                padded2d = twoDpadding(downsampled,filtersize);
            }
            else if(i>0){
                sigma = k*sigma;
                cout << sigma << endl;
                padded2d = twoDpadding(normalized,filtersize);
            }
            ...
            String img_name = "image " + string(" level ")+to_string(j)+ string(" intermediate_level ")+to_string(i);
